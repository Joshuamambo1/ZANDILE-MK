...
zandile({
  ...
  try {
    ...
    while (remainingPlayers.length > 1) {
      ...
      await conn.sendMessage(m.chat, { text: `🔔 ${isGreenLight ? "🟢 Green Light! Talk now!" : "🔴 Red Light! Stay silent!"}` });
    ...
    if (remainingPlayers.length === 1) {
      ...
      await conn.sendMessage(m.chat, { text: "No winners this time, everyone was eliminated!" });
    }
    ...
    const onGuess = (msg) => {
      if (msg.messages[0].key.remoteJid === m.chat) {
        ...
        if (!isNaN(guess)) {
          tries--;
          if (guess === target) {
            conn.sendMessage(m.chat, { text: `🎉 Congrats @${msg.messages[0].key.participant.split("@")[0]}! You guessed the number ${target} correctly!` });
            conn.ev.off("messages.upsert", onGuess);
            activeGames.delete(m.chat);
          } else if (tries === 0) {
            conn.sendMessage(m.chat, { text: `❌ Game over! The number was ${target}. Better luck next time!` });
            conn.ev.off("messages.upsert", onGuess);
            activeGames.delete(m.chat);
          } else {
            let hint = guess > target ? "Too high!" : "Too low!";
            conn.sendMessage(m.chat, { text: `❌ Wrong guess. ${hint} Tries left: ${tries}` });
          }
        }
      }
    };
