...
📌 *Note*: Some commands are owner-only`;
        ...
        await conn.sendMessage(
            from,
            {
                image: { url: `https://files.catbox.moe/jog59s.jpg` }, // Replace with privacy-themed image if available
                caption: privacyMenu,
                contextInfo: {
...
async (conn, mek, m, { from, isOwner, reply }) => {
    ...
    try {
        ...
        reply(`📋 \`ZANDILE MK BLOCKED USERS (${count})\`:\n\n${list}`);
    } catch (err) {
        console.error(err);
